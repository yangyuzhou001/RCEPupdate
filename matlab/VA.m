function [VAX, RDV, E, DVX, FVA] = VA(PIj_m_ni, Xj_f_ni, Yj_ni, tau_m, tau_f, B, G, J, N)
...
%     max(abs(VA9./sum(va9)'-1))
    ...
    Xik = ((eye(N*J) - ajk_ni.*kron(eye(N),ones(J,J)))^(-1))*(sum(C.*kron(eye(N),ones(J,1)),2) + reshape(Eik,J*N,1));
    DVXstar = sum(reshape((((bjk_ni.*(1 - kron(eye(N),ones(J,J))))*...
        (ajk_ni.*(1 - kron(eye(N),ones(J,J))))).*...
        (1 - kron(eye(N),ones(J,J))))*Xik,J,N).*B)';      
    
    RDV = VA4 + VA5;         
    DVX = VA3 + VA4 + VA5 + VA8 + DVXstar;
    FVA = VA6 + VA7 + VA9;
end
